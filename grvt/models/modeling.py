...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        # self.patch_cross = Conv2d(in_channels=config.hidden_size,
        #                                out_channels=config.hidden_size,
        #                                kernel_size=patch_size,
        #                                stride=patch_size)
...
class AttentionMix(nn.Module):
    ...
    def forward(self, x,topn):
        ...
        # maxk0=max((98,))
        # maxk1=max((24,))
        # _,pred0=last_map0.topk(maxk0,dim=2)
        # _,pred1=last_map1.topk(maxk1,dim=2)
        # last_map0=pred0.reshape(batchsize,12*98)
        # last_map1=pred1.reshape(batchsize,12*24)
        maxk=max((topn,))
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # encoded2 =self.encoder_norm2(hidden_states)# 前面的
        topn=3
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.contra_loss=config.contra_loss
    ...
    def forward(self, x, labels=None):
        ...
        #     logits_scale = self.head(x[:,197]) # 不用mix模块是197，用了是1
